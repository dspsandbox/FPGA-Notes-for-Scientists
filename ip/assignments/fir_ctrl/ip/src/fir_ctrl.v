module fir_ctrl #(
    ...
    output [RELOAD_WIDTH - 1 : 0] m_axis_reload_tdata,
    output reg m_axis_reload_tvalid,
    output m_axis_reload_tlast
);
    
    wire [1:0] regOut00, regOut01;
    reg [1:0] regOut00_old, regOut01_old;
    ...
    reg_bank_0 reg_bank_instance (
        ...
        .regOut00(regOut00),                
        .regOut01(regOut01),
        .regOut02(m_axis_config_tdata),                
        .regOut03(m_axis_reload_tdata)              
    );

    assign m_axis_config_tlast = regOut00[1];
    assign m_axis_reload_tlast = regOut01[1];
    ...
    always @(posedge clk) begin
        if(resetn==0) begin
            ...
            regOut01_old <= regOut01;
            if((regOut00_old[0]==0) && (regOut00[0]==1)) begin
                m_axis_config_tvalid <= 1;
            end else begin
                m_axis_config_tvalid <= 0;
            end 
            if((regOut01_old[0]==0) && (regOut01[0]==1)) begin
                m_axis_reload_tvalid <= 1;
            end else begin
                m_axis_reload_tvalid <= 0;
            end 
        end
